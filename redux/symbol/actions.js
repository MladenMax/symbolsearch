...
const updateNewsInit = () => {
	return {
		type: TYPES.UPDATE_NEWS_INIT
	}
}

const updateNewsError = error => {
	return {
		type: TYPES.UPDATE_NEWS_ERROR,
		error
	}
}

const updateNewsEnd = newsUpdate => {
	return {
		type: TYPES.UPDATE_NEWS_END,
		newsUpdate
	}
}

export const updateNews = (limit, offset, tag) => {
	return dispatch => {
		dispatch(updateNewsInit());
		return get(`applications/${apiConfig.applicationId}/news/coinpulse`, {
			limit,
			offset,
			tag
			...
			.then(res => {
				return dispatch(updateNewsEnd(res.results));
			})
			.catch(error => {
				return dispatch(updateNewsError(error));
			});
	}
}